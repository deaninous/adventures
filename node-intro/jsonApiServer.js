...
var http = require('http'), url = require('url'), q = require('querystring')



var server = http.createServer(function(req, res){
  res.writeHead(200, {"Content-Type" : "application/json"})
  res.end(JSON.stringify(sendResponse(req)))
}).listen(process.argv[2])
console.log(JSON.stringify({'hello': "hello"}))

function sendResponse(req){
  var parsedInput = url.parse(req.url)
  console.log(parsedInput.pathname)
  if(!parsedInput.path.match(/^\/api\/(parsetime|unixtime)\?iso\=.+/)){
    return [{'Please use a valid route': 'bla'},
            {'Request time': '/api/parsetime?iso="<your iso date>"'},
            {'Request nerd time': '/api/unixtime?iso="<your iso date>"'}]
  }else{
    givenIso = q.parse(parsedInput.query).iso
    var d = new Date(givenIso)

    if(/parsetime/.test(parsedInput.pathname)){
            return {
              'hour' : d.getHours(),
              'minute' : d.getMinutes(),
              'second' : d.getSeconds()
